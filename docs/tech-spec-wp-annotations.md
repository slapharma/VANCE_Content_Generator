...
- **Whole-page coverage (beyond spec):** the annotatable root is not just `.oped-article-body`. The plugin wraps the entire page — nav, sidebar, footer, body — via the theme's existing `wp_body_open`/`wp_footer` hooks (a `display:contents` wrapper so layout is untouched), with **zero theme edits**. This also fixed custom page templates (e.g. the GI Health hub) that never call `the_content()` and so could never have been covered by a `the_content` filter.
  - **Known limitation:** the homepage runs in "latest posts" mode (no single post ID) and is not annotatable. Every other singular view is. **Resolved 2026-07-09 — see §B.5.**
...
### B.5 Session update (2026-07-09) — image annotation went from "in-article only" to "everywhere," homepage coverage gap closed

Four deploys, each verified live (`php -l`/`node --check` + server-side `grep`/`php -l` after every tar-over-SSH push, cache purged after each). No local WP instance existed for this session either — same verification posture as B.4.

**1. Reply-threading bug fix.** A reply left on an annotation wasn't reaching either the mechanically-generated to-do or the AI-edit prompt — only the top-level comment was. Fixed with `VHH_Annotation_Store::thread_text()` (comment + its replies, flattened to one text blob) and `quote_for()` (anchor description generalized across text/image/insertion/overall), wired into `class-vhh-todos-cpt.php`'s to-do generator, `class-vhh-apply.php`'s AI-edit prompt, and the REST `export()` `merged` format. The `/extract-comments` skill doc was updated to fold each annotation's `replies[]` into its citation too — the fix needed to land in three independent consumers, not one.

**2. Image commenting silently failed on linked images.** Root cause (found by a background investigation, not guessed): WordPress's "Link To: Media File/Attachment Page" option wraps `<img>` in `<a>`. `preventDefault()` on `pointerdown` does **not** suppress the browser's own click-driven navigation — only a `click`-event `preventDefault()` does. A plain click both opened the save popover and navigated away before the save could be seen to work. **General gotcha for any overlay-based annotation system:** guard the `click` event specifically when the overlay may sit inside a native `<a>`, not just `pointerdown`/`pointerup`.

**3. New target type: insertion points ("comment on empty space").** `InsertionPointSelector { prefix, suffix }` — zero-width caret position between two content blocks, anchored the same way TextQuoteSelector's context is (tail of the block before / head of the block after) but with no `exact` span. UX: hover the gap between paragraphs in commenting mode → a "+" button appears → same shared popover. Client-side re-anchoring on reload scores every current block-boundary gap by prefix/suffix similarity (small dedicated matcher, not the full char-offset engine — this only ever matches at block boundaries). New `assets/js/insertion-annotator.js`, new toggle in the settings UI (on by default). Needed **zero changes** to to-do generation or the AI-edit prompt — `quote_for()` had been written generically enough in change #1 to already render `"Insertion point: …prefix ⟨INSERT HERE⟩ suffix…"` for this type.

**4. Image annotation extended from `<img>` tags to CSS `background-image` containers, and from "same post only" to "cards linking to a different post."** This is the section that resolves the B.1 homepage limitation:
   - **Background-image commenting:** several "images" (hero banners, some thumbnails) are `background-image` `<div>`s, not `<img>` tags — `querySelectorAll('img')` never finds them. Extended `image-annotator.js` to also discover a **hardcoded class allowlist** of known content-image containers (deliberately not a blanket `getComputedStyle` scan, which would catch decorative chrome backgrounds never meant to be commentable) and reuse the exact same overlay/region/pin machinery, branching only on how the reference `src` is extracted (regex the *last* `url(...)` out of computed `background-image`, since a gradient overlay is often layered first).
   - **Cross-post card commenting:** many "images" are actually whole clickable **cards linking to a different post** than the page you're on (a homepage "latest content" card, a Related Articles card on an article page). New `card-annotator.js`: whole-card click (no drag — feedback on the component, not a sub-region), posting to a **client-supplied `post` ID read from a `data-vhh-post-id` attribute added to the card markup** — NOT `cfg.postId`. This needed **zero REST/selector schema changes**: `target_type:'image'` + `ImageRegionSelector` already accepted an arbitrary `src` and never required a literal `<img>` or same-post attachment; `create_from_request()` already accepted whatever `post` ID the client sent, gated only by the post-type allowlist. The sidebar's existing "cross-page" card rendering (`a.post !== cfg.postId`) already handled display with no changes. **Real bug caught mid-implementation:** a card's own thumbnail/background must be *excluded* from the generic `<img>`/background discovery in #1 above, or two overlays stack (a wrongly-attributed one from the generic pass, hidden underneath the correctly-attributed card-level one) — kept a shared exclusion selector between the two files.
   - **Homepage/category listing coverage:** the front-end gate (`is_singular()`) was extended to `is_front_page() || is_home() || is_category()`, since card comments don't depend on the page having its own post. This surfaced a **site-specific config gotcha, not a code bug:** this particular site's Reading setting is "latest posts" (no static front page), so `get_queried_object()` genuinely returns `null` there — the gate change was reachable but a no-op until `client_config()` was also relaxed to accept a non-`WP_Post` (`null`, or a `WP_Term` on a category page) and default `postId` to `0` in that case.
   - **The "postId=0 dead end" UX trap, and the fix:** once card comments worked but `cfg.postId` was `0`, text-selection, "+Overall feedback," and direct image commenting were all still *offered* on the homepage (the pill still appeared, the button still rendered) but every save failed server-side with "This post cannot be annotated" — confusing, since nothing signaled up front that only cards worked there. Fixed by gating those three client-side entry points on `!!cfg.postId`, so unsupported affordances simply don't render rather than failing after the fact.
   - **General homepage feedback, closing the loop:** the user then asked for the hero/CTA text and footer links (which had just been correctly-but-confusingly disabled) to actually work. Rather than inventing parallel storage for "feedback not about any post," gave the homepage a **single hidden internal post** to serve as its identity: a new `vhh_site_feedback` post type (`public => false`, `publicly_queryable => false`, excluded from search/REST/nav-menus/customizer-type-list), one row lazily created and cached in an option (`class-vhh-site-feedback.php`). `enqueue()` resolves this post whenever `is_front_page() || is_home()` has no real `WP_Post`; `post_type_allowed()` special-cases the internal type to always pass. **This one change made every phase-3 guard self-heal with zero further client-side edits** — once `cfg.postId` is a real ID again, all the `if (!cfg.postId) return;` guards stop suppressing anything. `post_brief()` needed one special case: the internal post's own permalink doesn't resolve (not publicly queryable), so cross-page sidebar links for it go to `home_url('/')` instead.
   - **Reusable pattern for FBTR/Feedback-module parity work:** *"a hidden, `public => false` CPT as the attachment point for comments/feedback that aren't about any specific piece of content"* is a general technique worth carrying into the Feedback module design if FBTR needs to support non-singular views (archives, a shop page, a landing page) — same `wp_comments`-based storage, same isolation/moderation code paths, no schema fork.
   - **Category pages stay card-only** (deliberately, per explicit scope) — no synthetic post was created per-category; extending this pattern there (one bucket post per term) would be a small, symmetric follow-up.
   - **Parallel-session hazard, worth carrying into any multi-agent WP workflow:** this site's theme deploy command tars the *entire* theme directory, not just changed files. A second, concurrent Claude Code session was independently editing unrelated theme files (a card-clickability CSS pattern, a homepage layout tweak) in the same working tree with no git-worktree isolation. Before any deploy that packages a whole directory rather than a diff, check for other running sessions on the same `cwd` (session-list tooling) and get explicit sign-off before bundling their in-flight, unreviewed work into a production push.
