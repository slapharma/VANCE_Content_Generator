 ...
 * passport-report.js  v1.1.0
 *
 * ┌───────────────────────────────────────────────────────────────────────┐
 * │  VENDORED FILE — copy this into each consuming app as                 │
 * │  lib/passport-report.{js,mjs}. The master lives in VANCE-HQ.          │
 * │  Change it there, bump the version, re-copy. Drift is caught by       │
 * │  VANCE-HQ/scripts/check-vendored.mjs.                                 │
 * └───────────────────────────────────────────────────────────────────────┘
 *
 ...
 * @param {string} asset this app's id in VANCE-HQ/lib/estate.js — 'content' | 'cs' | 'alerts'
...
export function reportPassportUse(req, asset, env = process.env) {
  ...
  try {
    ...
    const base = (read(env.VANCEHQ_URL) || read(env.VNET_URL) || HQ_DEFAULT).replace(/\/+$/, '');
