...
// the right content shape (e.g. Vance Living for patient-facing posts, not Clinical Review).
const GASTRO_LIVING_PROMPT = `Vance Living_Generation Prompt
...
Target 650 to 1000 words (body only)
...
STRUCTURE
1. Headline (~10 words) — MUST start with "Vance Health News: " followed by a clear, informative phrase. No clickbait.
...
OUTPUT FORMAT
Output the article in markdown. The very first line MUST be the title prefixed with "# Vance Health News: ..." All other section headers use "## ". Do NOT use "Clinical Review", "IBD Industry News", "IBD Health News", or any other variant — always start the title with "Vance Health News: ".
...
// Used when a Vance Living rule's source is a title-only xlsx upload (no URLs to fetch).
...
const CATEGORY_TITLE_PREFIXES = {
  ...
  'industry-news':    'Vance Health News: ',
  'white-papers':     'White Paper: ',
  'op-eds':           'Op-Ed: ',
  'ibd-living':       'Vance Living: ',
};
...
const ANY_CATEGORY_PREFIX_RE = /^(clinical review|gastro living|vance living|gastro health news|vance health news|op[-\s]?ed|white paper)\s*:\s*/i;
...
async function generateArticle(item, rule, fetchFn = fetch) {
  ...
  // Title-only items (xlsx column A with no hyperlink) get the Vance Living Blog Master
...
export default async function handler(req, res, { fetchFn = fetch } = {}) {
  ...
  for (const rule of dueRules) {
    try {
      ...
      // On Vercel Pro (300s maxDuration), each article costs ~15–25s (LLM)
      // + ~15–25s (hero image) + ~5s (notify) ≈ 40–55s. We cap at 6 to keep
      // a safety margin under the 300s budget.
      ...
      const max = Math.min(requestedMax || 0, 6);
      ...
      for (const item of toProcess) {
        ...
        if (item.sourceType === 'upload') {
          for (const src of rule.sources) {
            ...
            if (item.titleOnly) {
              if (Array.isArray(src.titlesOnly) && src.titlesOnly.includes(item.title)) {
        ...
        if (rule.generation.heroImage !== false) {
          ...
          try {
            const imageData = await Promise.race([
              generateImageFast(generated.title, '16:9'),
