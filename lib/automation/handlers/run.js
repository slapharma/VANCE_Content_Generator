...
import { generateImageFast, generateHeroStockImage } from '../../social/media.js';
...
// Default when a rule stores no model of its own. Deliberately a PAID model:
// a `:free` slug here is a slug OpenRouter can withdraw without notice, which
// is exactly how the Industry News rule broke on 2026-08-18. Free models keep
// their place in PAID_FALLBACKS, where a withdrawal degrades instead of fails.
const DEFAULT_MODEL = 'anthropic/claude-sonnet-5';
const DEFAULT_PROMPT = `You are a medical writer creating a curated clinical paper review for publication on Vance Health Hub (vancehealthhub.co.uk), the IBD and gastroenterology content platform owned by Vance Medical Foods Ltd. Produce a structured, objective summary of a single published clinical trial intended for a physician audience. Your role is purely curatorial: you present what the paper reports, nothing more. You do not add clinical commentary, personal opinion, practice recommendations, or conclusions beyond those stated by the authors.
...
TASK
Write a complete, publication-ready blog article for Vance Health Hub (vancehealthhub.co.uk), the IBD and gastroenterology content platform owned by Vance Medical Foods Ltd, based on the topic.
The article must be:
...
const INDUSTRY_NEWS_PROMPT = `You are a medical journalist writing for Vance Health Hub — a UK gastroenterology and GI health digital research platform (covering the full breadth of GI medicine, with particular interest in IBD, IBS, and related conditions). Transform the source material (a clinical paper, research summary, or healthcare press release) into a clear, engaging Healthcare News article for vancehealthhub.co.uk.

Vance Health Hub's focus is IBD (Crohn's disease, ulcerative colitis), gastrointestinal nutrition, and longevity science. Readers include UK-based gastroenterologists, IBD nurses, dietitians, and GPs, as well as well-informed patients living with IBD and gastrointestinal conditions.
...
STRUCTURE
1. Headline (~10 words) — a clear, informative phrase describing what happened. No clickbait. Do NOT open the headline with a category label such as "Health News", "Healthcare News", or "Industry News".
...
6. Reference — full citation in Vancouver format with DOI where available. Cite the ultimate source, not the outlet that reported it — see SOURCE PRIORITY below.

SOURCE PRIORITY (REFERENCES)
Always reference the ultimate source, never the intermediary that reported it. The source material supplied to you is often a secondary write-up — a news site, aggregator, university media office, or press release — that is itself reporting on a primary publication. Follow the chain back and cite the original.
Trace every claim to its origin: the secondary report almost always names or links the underlying paper, trial, dataset, guideline, or regulatory decision. Identify that and cite it.
Preferred order: peer-reviewed journal article (with DOI) > preprint or conference abstract > trial registry record, regulatory decision, or official guideline > institutional press release > news article reporting on any of the above.
Give the full primary citation — authors, article title, journal, year, volume(issue), pages, and DOI. Do not cite a bare URL when a DOI or full citation exists, and cite the publisher's own record in preference to a mirror or a PDF hosted elsewhere.
Worked example: if the source material is a MedicalXpress news story about a paper in Cellular and Molecular Gastroenterology and Hepatology, the reference is the CMGH paper itself — not the MedicalXpress page.
Only cite a news outlet or press release when it IS the primary source, i.e. it reports something with no underlying publication (a company announcement, a regulatory statement, an interview). Where the secondary report contributes something of its own — an expert comment, context absent from the paper — you may cite it in addition to the primary source, clearly attributed and listed after it.
If the underlying primary source cannot be identified from the material provided, say so plainly in the Reference section rather than presenting the secondary report as though it were the original research.
Never fabricate a citation, DOI, journal name, volume, or page range. Omit any detail you cannot establish from the source material rather than inventing it.

OUTPUT FORMAT
Output the article in markdown. The very first line MUST be the title prefixed with "# " (no other text on the first line). Do NOT add a category prefix to the title — no "Gastro Health News", "Vance Health News", "Healthcare News", "Industry News", "Clinical Review", or any other variant. The headline stands on its own. All other section headers use "## ".
...
const WHITE_PAPERS_PROMPT = `You are a scientific medical writer producing a White Paper for Vance Health Hub — a UK gastroenterology and GI health research platform. Synthesise multiple clinical papers and published evidence sources into a comprehensive, structured white paper on a specific topic in IBD, gastrointestinal nutrition, or gastrointestinal medicine.
...
const INFOGRAPHICS_PROMPT = `You are a medical content designer producing infographic copy for Vance Health Hub — a UK gastroenterology and GI health digital platform. Transform the source clinical paper or research summary into structured, visually optimised copy for an infographic.
...
// Category prefixes (e.g. "Gastro Living: ", "Clinical Review: ", "Vance Health
// News: ") were historically prepended to article titles. They are no longer wanted
// — strip any known prefix from the title, regardless of category. The category
// prompts (both the hardcoded defaults and any user-customised KV prompts) may still
// instruct the LLM to add one, so we remove it server-side after the LLM returns.
// Matches case-insensitively with optional whitespace around the colon; the brand
// word is optional and interchangeable (gastro/vance/ibd) because rebrands leave
// older KV-saved prompts asking for the previous label.
const ANY_CATEGORY_PREFIX_RE = /^(?:(?:vance|gastro|ibd)\s+(?:living|health(?:care)?\s+news)|clinical review|health(?:care)?\s+news|industry news|op[-\s]?ed|white paper|infographic)\s*:\s*/i;

function stripCategoryPrefix(title) {
  if (!title) return title;
  return title.replace(ANY_CATEGORY_PREFIX_RE, '').trim();
}

// Drop any "Reading Time" meta line the LLM emits (usually right under the title)
// so it never persists in the stored article body. The master prompt still asks
// for it on some installs; this guarantees it's gone regardless.
function stripReadingTimeLines(md) {
  if (!md) return md;
  return md.split('\n').filter(l => !/^\**\s*reading time\b/i.test(l.trim())).join('\n').trimStart();
}
...
// ── Industry News animal-story exclusion ──────────────────────────────────────
// Industry News (Healthcare News) rules pull from general medical/press RSS and
// Gmail sources, which routinely mix in veterinary or animal-model research
// (e.g. "in mice") that doesn't belong on a clinician/patient-facing human
// health feed. Excluded before generation so no article is ever drafted from
// an animal-related source item.
const ANIMAL_RE = /\b(mice|mouse|murine|rats?|rodents?|guinea[\s-]?pigs?|rabbits?|primates?|monkeys?|macaques?|chimpanzees?|dogs?|canine|cats?|feline|pigs?|porcine|swine|livestock|cattle|cows?|bovine|sheep|ovine|goats?|horses?|equine|poultry|chickens?|zebrafish|drosophila|fruit\s?flies|animal\s?models?|animal\s?stud(?:y|ies)|veterinary|vet\s?clinic|pets?|wildlife|farm\s?animals?)\b/i;

export function mentionsAnimals(item) {
  const text = [item.title, item.rawText, item.feedDescription].filter(Boolean).join(' \n ');
...
// reviews land in the right child term in WordPress. Keyed by app category, then
// by lower-cased prompt name. Values are display names whose WP slug (Clinical
// Trial Abstracts → clinical-trial-abstracts) the publish endpoint resolves.
// Includes both the current preset names ("Clinical Trial Abstract"/"Clinical
// Trial Review") and the older "Clinical Abstract Patient"/"Clinical Abstract
// Practitioner" names, kept so any rule saved before the 2026-07-09 preset
// rename still routes correctly until it's re-saved.
const PROMPT_SUBCATEGORY_MAP = {
  'clinical-reviews': {
    'clinical trial abstract':        'Clinical Trial Abstracts',
    'clinical trial review':          'Clinical Trial Reviews',
    'clinical abstract patient':      'Clinical Trial Abstracts',
    'clinical abstract practitioner': 'Clinical Trial Reviews',
  },
};

// Resolve the WP sub-category implied by a rule's selected prompt preset, or null.
// Exact map match first; for clinical-reviews fall back to a forgiving heuristic
// (current "trial abstract"/"trial review" naming, plus the legacy patient/
// practitioner naming) so preset-name variants still route (e.g. "Clinical
// Trial Abstract - v2", "Clinical Abstract - Patient"). Note: a bare "review"
// substring is deliberately NOT matched — the parent category itself is named
// "Clinical Reviews", so that word alone would false-positive on almost every
// prompt name in this category.
function subCategoryForRulePrompt(rule) {
  ...
  if (catId === 'clinical-reviews') {
    if (name.includes('trial review') || name.includes('practitioner')) return 'Clinical Trial Reviews';
    if (name.includes('trial abstract') || name.includes('patient')) return 'Clinical Trial Abstracts';
  }
  return null;
}
...
// Ordered by descending quality, every entry verified live on OpenRouter.
// Claude Haiku leads because DEFAULT_MODEL is now Sonnet 5: when the primary is
// a Claude model, the first fallback should stay in the family rather than
// cliff-drop to a small open model halfway through a clinical article.
// `openai/gpt-oss-120b:free` and `z-ai/glm-4.5-air:free` sat here until
// 2026-08-18 and had both been retired by OpenRouter — the chain read as four
// layers of protection and was two. See scripts/check-model-slugs.mjs.
...
//
// Rate limits were the original case. The second is a model-availability 404:
// OpenRouter withdraws a model's free tier and then answers `:free` requests
// with "This model is unavailable for free. The paid version is available now -
// use this slug instead: <paid slug>" — which is exactly the model already
// sitting next in the chain. Matching only "no endpoints" meant that reply was
// treated as fatal, so generation died one step short of the fallback that
// would have worked (seen in production 2026-08-18 on the Industry News rule
// "NewsNow - Gastro", whose null model resolves to DEFAULT_MODEL). The social
// module hit the identical failure on 2026-07-27; this predicate now matches
// lib/social/llm.js so the two modules degrade the same way.
//
...
export function shouldRetryWithNextModel(data) {
  const code = data.error?.code;
  const msg = (data.error?.message || '').toLowerCase();
  if (code === 429 || msg.includes('rate-limit') || msg.includes('rate limit')) return true;
  // Model-availability errors — OpenRouter retired the model or withdrew its
  // free tier; try the next one.
  if (msg.includes('no endpoints')
      || msg.includes('unavailable for free')
      || msg.includes('use this slug instead')
      || msg.includes('no allowed providers')) return true;
    ...
    for (const model of buildFallbackChain(primaryModel)) {
      ...
      if (text) {
        ...
        if (lines[0].startsWith('#')) {
          title = stripCategoryPrefix(lines[0].replace(/^#+\s*/, '').trim());
...
async function generateArticle(item, rule, fetchFn = fetch) {
  ...
  if (isTitleOnly) {
    ...
    // Any category title-prefix (e.g. "Gastro Living: ") emitted by the LLM is
    // stripped server-side by stripCategoryPrefix() after the LLM returns.
  ...
  for (const [idx, model] of chain.entries()) {
    ...
    if (text) {
      ...
      title = stripCategoryPrefix(title);
      body = stripReadingTimeLines(body);
...
export default async function handler(req, res, { fetchFn = fetch } = {}) {
  ...
  for (const rule of dueRules) {
    ...
    try {
      ...
      // Healthcare (Industry News) rules must never generate from animal-related
      // stories — see ANIMAL_RE / mentionsAnimals() above.
      if (rule.category === 'industry-news' && sourceItems.length) {
        const excluded = sourceItems.filter(mentionsAnimals);
        if (excluded.length) {
          sourceItems = sourceItems.filter(it => !mentionsAnimals(it));
          await writeLog({
            ruleId: rule.id, ruleName: rule.name, level: 'info',
            message: `Excluded ${excluded.length} animal-related item(s) before generation: ${excluded.map(i => i.title).slice(0, 5).join('; ')}${excluded.length > 5 ? '…' : ''}`,
          });
        }
      }
      ...
      // Resolve the hero image prompt template + source for this rule's category
      // once per run (per-category override wins, else the global default).
      // undefined template => media.js falls back to its built-in default.
      // heroImageSource 'stock' => Pexels stock photo; otherwise AI generation.
      let heroPromptTemplate;
      let heroImageSource = 'ai';
      let stockProvider = 'pexels';
      let unsplashKey = '';
      try {
        ...
        if (hp) {
          ...
          if (hp.sources && hp.sources[rule.category] === 'stock') heroImageSource = 'stock';
          if (hp.stockProvider === 'unsplash') stockProvider = 'unsplash';
          if (typeof hp.unsplashKey === 'string') unsplashKey = hp.unsplashKey;
        }
      } catch (_) { /* non-fatal — fall back to media.js default */ }

      // WP sub-category for this rule. The explicit rule.generation.subCategory
      // (set directly in the wizard) always wins; the prompt-preset-name heuristic
      // is a fallback only, for legacy rules that never set it explicitly — it's
      // fragile because it silently breaks if the prompt preset is renamed.
      // Per-item upload metadata still wins over both when present.
      const rulePromptSubCategory = (rule.generation?.subCategory || '').trim() || subCategoryForRulePrompt(rule);
      ...
      for (const item of toProcess) {
        ...
        if (rule.generation.heroImage !== false) {
          ...
          try {
            // Per-category source: Pexels stock photo or AI generation.
            const heroJob = heroImageSource === 'stock'
              ? generateHeroStockImage(generated.title, { provider: stockProvider, unsplashKey })
            ...
            if (imageData?.url) {
              // imageData.provider reflects who actually served the image — generateHeroStockImage
              // can silently fall back from unsplash to pexels (e.g. rate-limit), so trust it over
              // the requested stockProvider rather than assuming the original provider won.
              const heroType = heroImageSource === 'stock' ? (imageData.provider === 'unsplash' ? 'unsplash' : 'pexels') : 'ai';
              // Stock-photo attribution, persisted so the publish step can append
              // the credit line the Unsplash API guidelines require.
              const heroCredit = heroImageSource === 'stock' && imageData.photographer ? {
                photographer: imageData.photographer,
                photographerUrl: imageData.photographerUrl || null,
                photoUrl: imageData.sourceUrl || null,
                provider: heroType,
              } : null;
              // Provider photo id, kept so the one-use-only stock ledger can identify
              // this photo exactly rather than inferring it from the image URL.
              const heroPhotoId = heroImageSource === 'stock' ? (imageData.id || null) : null;
              const updated = { ...content, heroImageUrl: imageData.url, heroImageType: heroType, heroImageCredit: heroCredit, heroImagePhotoId: heroPhotoId, updatedAt: now };
              await kv.set(`content:${content.id}`, updated);
              content.heroImageUrl = imageData.url;
              content.heroImageType = heroType;
              content.heroImageCredit = heroCredit;
              content.heroImagePhotoId = heroPhotoId;
            }
          } catch (imgErr) {
        ...
        // 4e. The Instagram Article Carousel used to be built here, between the
        // hero image and the review/publish branch. It is not any more: a deck
        // built at generation time is a snapshot of a draft, and every hero swap
        // or copy edit made during review left it advertising an article that no
        // longer matched. It now runs off the back of a successful WordPress
        // publish instead — see lib/social/carousel-on-publish.js, hooked into
        // api/publish, which covers this rule's auto-publish path, the
        // approve-on-review path, the scheduled cron sweep and the manual button
        // with one implementation. `rule.generation.articleCarousel` and the
        // carousel* settings still drive it; they are just read later.
