...
// Category prefixes (e.g. "Gastro Living: ", "Clinical Review: ") were historically
// prepended to article titles. They are no longer wanted — strip any known prefix
// from the title, regardless of category. The category prompts (both the hardcoded
// defaults and any user-customised KV prompts) may still instruct the LLM to add one,
// so we remove it server-side after the LLM returns. Matches known prefixes
// case-insensitively with optional whitespace around the colon.
const ANY_CATEGORY_PREFIX_RE = /^(clinical review|gastro living|gastro health news|op[-\s]?ed|white paper|infographic)\s*:\s*/i;
