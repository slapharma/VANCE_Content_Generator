 ...
 * │  Do not edit a copy in place. Change it here in the VANCE-HQ repo,    │
 * │  bump the version above, and re-copy to every app. When two copies    │
 ...
 * ── What this is for ─────────────────────────────────────────────────────
 *
 * VANCE-HQ signs one cookie, scoped to .vancemedicalfoods.co.uk, that every
 ...
 * network call to VANCE-HQ, ever. That is the whole point, and it is not an
 ...
 * VANCE-HQ, then VANCE-HQ would become a thing that can take the
 * dashboard down at exactly the moment somebody opens it to find out what
 * is wrong.
 *
 * ── How to wire it in ────────────────────────────────────────────────────
 *
 ...
 *   3. Neither → 401, or redirect to VANCE-HQ's sign-in.
 *
 ...
 * ── Configuration ────────────────────────────────────────────────────────
 *
 *   VANCE_SSO_SECRET       required. Identical across VANCE-HQ and every app.
...
export function signinUrl(req, env = process.env) {
  // New name first, old name second: this file ships into apps whose
  // Vercel projects may still carry VNET_URL. See VANCE-HQ/lib/env.js.
  const configured = read(env.VANCEHQ_URL) || read(env.VNET_URL);
  const base = (configured || 'https://hq.vancemedicalfoods.co.uk').replace(/\/+$/, '');
