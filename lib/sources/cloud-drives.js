...
export async function fetchGoogleDrive(source, lastRunAt, fetchFn = fetch, opts = {}) {
  ...
  // This caps how many files we OCR/generate from per run — NOT how many we list
  // (see LIST_PAGE_SIZE below).
  ...
  // How many files to list per query before filtering out already-consumed ones.
  // Must stay well above MAX_FILES_PER_RUN: with orderBy=modifiedTime desc, listing
  // only MAX_FILES_PER_RUN and THEN filtering consumed ids meant that once the N
  // most-recently-modified files in a folder were all consumed, older unprocessed
  // files further down the folder were never even seen — the run silently reported
  // 0 source items despite unconsumed files existing. Matches the pageSize used by
  // the "Re-process selected" file picker (sources-list.js) so both paths see the
  ...
  const listUrl = `https://www.googleapis.com/drive/v3/files?q=${encodeURIComponent(q)}&fields=files(id,name,mimeType,webViewLink,modifiedTime)&pageSize=${LIST_PAGE_SIZE}&orderBy=modifiedTime%20desc&corpora=allDrives&includeItemsFromAllDrives=true&supportsAllDrives=true`;
  ...
  const unconsumed = files.filter(f => !consumed.has(f.id)).slice(0, MAX_FILES_PER_RUN);
  if (!unconsumed.length) {
    // Same "don't leave operators guessing" rationale as the 0-files-from-Drive
    // branch above: surface exactly what was seen so a stale consumedFileIds
    // list, a listing-window gap, or a genuinely fully-processed folder can be
    // told apart from the logs alone instead of re-derived by hand each time.
    throw new Error(`Google Drive: listed ${files.length} file(s) in folder "${folderId}", all already in consumedFileIds (${consumed.size} tracked). Nothing new to process.`);
  }
