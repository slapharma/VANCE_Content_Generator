...
export async function fetchGmailMessages(source, lastRunAt, fetchFn = fetch) {
  ...
  // Search all mail by default (not INBOX-only). Digest emails like Google Alerts
  // are frequently archived — by the user or by a filter — before the automation
  // runs, so an INBOX-scoped search returns 0 even when matching mail exists. With
  // no labels pinned, Gmail searches everything except Spam/Trash; the `after:`
  // time bound (since lastRunAt, or the 7-day fallback) plus maxResults keep the
  // result volume bounded. A rule may still pin source.labelIds to narrow scope.
  const labelIds = source.labelIds?.length ? source.labelIds : [];
