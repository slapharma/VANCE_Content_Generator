 ...
 *
 * Every element is split, not just a bare string. An array element is not a
 * promise of a single tag: models routinely return the whole set in one element
 * ("#GutHealth #Nutrition #Vance"), and because the per-tag cleanup below strips
 * everything outside [A-Za-z0-9_], an unsplit element does not fail loudly — it
 * fuses into one unusable ninety-character tag. Observed on a live promo caption
 * 2026-08-04. Splitting on '#' as well as whitespace and commas catches the
 * run-together variant, and costs nothing: '#' cannot appear inside a tag, so a
 * leading one simply yields an empty field that the loop skips.
 *
...
export function normaliseHashtags(list) {
  ...
  for (const raw of source) {
    const tag = String(raw || '').trim().replace(/[^A-Za-z0-9_]/g, '');
    if (!tag) continue;
