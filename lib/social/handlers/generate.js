...
// callLLM with fallback: try free model first, fall back to paid on 429
...
async function callOpenRouter(prompt) {
  ...
  const envDefault = process.env.DEFAULT_LLM_MODEL || 'google/gemma-3-27b-it:free';
  // A/B harness: override primary with the picked variant when test is enabled
  const ab = await pickModelForGeneration(envDefault);
  const primaryModel = ab.model;
  const pickedVariant = ab.variant;

  // Build fallback chain: primary → paid equivalent → paid fallbacks
  ...
  for (const [idx, model] of chain.entries()) {
    ...
    if (text) {
      if (data.usage) {
        recordLlmUsage({
          model, usage: data.usage, source: 'social-generate',
          outcome: 'success',
    ...
    // First attempt failure for the picked A/B variant — record so failure rate is accurate
    if (isPickedVariant && data?.error) {
      recordLlmFailure({
        model, source: 'social-generate', variant: pickedVariant,
        failureReason: data.error?.message || data.error?.code,
      }).catch(() => {});
    }

    if (isRateLimited(data)) { lastError = `${model}: rate-limited`; continue; }
