...
//   SOCIAL_IMAGE_MODELS = comma-separated slugs, primary first.
// Defaults verified against OpenRouter /api/v1/models on 2026-05-15.
const MODEL_CHAIN = (process.env.SOCIAL_IMAGE_MODELS
  || 'google/gemini-3.1-flash-image-preview,openai/gpt-5.4-image-2'
).split(',').map(s => s.trim()).filter(Boolean);
const ALLOWED_MODELS = new Set(MODEL_CHAIN);
...
export default async function handler(req, res) {
  ...
  // Article heroes are landscape; a carousel cover is a 1080x1350 portrait slide,
  // and a 16:9 source cropped to 4:5 loses most of the subject. Whitelisted rather
  // than passed through so a typo cannot reach OpenRouter as an invalid config.
  // Defaults to 16:9, so every existing hero caller is unaffected.
  const ALLOWED_RATIOS = new Set(['16:9', '4:5', '1:1', '9:16']);
  const safeRatio = ALLOWED_RATIOS.has(aspectRatio) ? aspectRatio : '16:9';
  ...
  try {
    resp = await fetch(OPENROUTER_BASE, {
      ...
      body: JSON.stringify({
        ...
        image_config: { aspect_ratio: safeRatio },
      }),
    });
  } catch (err) {
