...
export default async function handler(req, res, action) {
  ...
  if (action === 'check') {
    ...
    try {
      ...
      // An expired token is amber, never red: the check itself did exactly what it
      // was for. Red here would mean "the expiry check stopped running", which is
      // the worse and quieter problem — it is what lets tokens lapse unnoticed.
      await heartbeat('cg-social-connections', {
        status: failures.length ? 'warn' : 'ok',
        message: failures.length
          ? `${failures.length}/${checked} failing: ${failures.map(f => f.label).join(', ')}`
          : `${checked} platform(s) OK`,
      });
    } catch (err) {
      console.error('[connections/check] fatal error:', err);
      await heartbeat('cg-social-connections', { status: 'fail', message: err?.message || 'check threw' });
      return res.status(500).json({ error: err?.message || 'Connections check failed' });
    }
