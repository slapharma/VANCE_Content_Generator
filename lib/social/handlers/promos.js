...
async function runNow(req, res, id) {
  const promo = await getPromo(id);
  if (!promo) return notFound(res, id);
  // `postNow` forces this single occurrence live regardless of the campaign's
  // standing post mode. A body flag rather than its own route because every
  // other thing about the run is identical — same copy resolution, same CSV
  // cursor advance, same occurrence count — and only the finished deck's
  // disposition differs. Defaults false, so the existing caller is unchanged.
  const postNow = req.body?.postNow === true;
  const result = await runPromoOccurrence(promo, { postNow });
