    ...
    for (const postRefId of dueIds) {
      ...
      try {
        ...
        if (isCarousel) {
          ...
          // Same spacing gate the immediate path uses: a cron run can find
          // several decks due at once, and posting them back to back is the
          // burst that Meta's crawler cannot keep up with. See post-gate.js.
          const gate = await acquirePostGate();
          if (!gate.gated) console.warn(`[cron] posting ${carousel.id} without the spacing gate — ${gate.reason}`);
