...
import { markStockUsed, coverAsStockPhoto } from './stock-ledger.js';
...
export async function savePromo(promo, { indexIt = false } = {}) {
  ...
  // A stock cover is spent once it belongs to a saved campaign — the same
  // one-use-only rule article heroes follow, and the reason it lives at this choke
  // point rather than in each of the four handlers that call savePromo.
  await markStockUsed(coverAsStockPhoto(record));
  return record;
}
