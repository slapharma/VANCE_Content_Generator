...
/**
 * Parse a pasted/uploaded CSV into `{ message, cta }` rows.
 *
 * Hand-rolled rather than pulled from a dependency because the input is two
 ...
 * doubled quotes as an escape, CRLF, and an optional header row.
 */
export function parsePromoCsv(text) {
  ...
  // Drop a header row if the first cell looks like a column name rather than copy.
  const first = (rows[0][0] || '').trim().toLowerCase();
  const body = ['message', 'text', 'copy', 'headline'].includes(first) ? rows.slice(1) : rows;
