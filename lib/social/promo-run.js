...
import { themeFor, PROMO_STYLE, FIXED_COPY } from './carousel-theme.js';
...
async function resolveCopy(promo) {
  ...
  if (promo.messagingMode === 'csv') {
    ...
    // `fields` is present only for a CSV that carried a header row. Those values
    // are literal and overwrite the generated copy (see applyCsvFields); a
    // headerless CSV leaves it empty and behaves exactly as it always has.
    const variation = { mode: 'csv', message: row.message, cta: row.cta, fields: row.fields || {} };
...
async function canvaSlideUrls(promo, spec) {
  ...
  if (promo.renderer === 'canva-template') {
    ...
    // An empty payload now means one of two things, and both are worth saying:
    // the template declared fields and none of them matched, or the generated
    // copy came back empty. When the template's fields are unknown (the usual
    // case — Canva's list endpoint omits the dataset) buildAutofillData sends
    // every name it holds, so a non-empty spec cannot produce an empty payload.
    // The CTA label is campaign config with a themed default, and the satori
    // renderer resolves that default at draw time (cta.label || theme.ctaLabel
    // || FIXED_COPY.ctaLabel). Autofill has no draw time: an unfilled field
    // keeps its placeholder text, so a campaign with no ctaLabel would publish
    // a chip reading literally "cta". Resolve the same chain here instead.
    const ctaLabel = spec?.cta?.label || themeFor(PROMO_STYLE).ctaLabel || FIXED_COPY.ctaLabel;
    const filled = { ...spec, cta: { ...spec?.cta, label: ctaLabel } };
    const data = buildAutofillData(filled, promo.canvaFields || []);
    if (!Object.keys(data).length) {
      throw new Error('Nothing could be autofilled: the generated copy was empty, or the brand template declares field names that do not match any of hookTitle, brief, point1, point1body, update, cta or domain.');
    }
 ...
 */
export async function runPromoOccurrence(promo, { dryRun = false, postNow = false } = {}) {
  ...
  try {
    ...
    //
    // `postNow` is the one-off override behind the builder's "Build & post now":
    // this occurrence goes live whatever the campaign's standing mode says, and
    // the campaign itself is not touched. The override is stamped onto the deck
    // rather than passed beside it so the record explains its own fate — a deck
    // that went out immediately reads as `immediate` forever after, regardless
    // of what the campaign is set to today.
    const forPosting = postNow ? { ...hosted, postMode: 'immediate' } : hosted;
    const schedule = hosted.status === STATUS.ready
      ? await applyPostMode(forPosting)
