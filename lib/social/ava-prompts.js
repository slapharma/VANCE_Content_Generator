// Brand config — resolved from env vars at module load.
// Each Vercel project (one per brand fork) sets these once; defaults match the
// original Vance Health Hub deployment for backwards compatibility.
export const BRAND = {
  name:            process.env.BRAND_NAME            || 'Vance Medical Foods',
  niche:           process.env.BRAND_NICHE           || 'clinical nutrition for medical foods',
  // The public-facing social handle is the Health Hub, not the parent company —
  // consistent with the CTA domain (vancehealthhub.co.uk) and the house rule that
  // Vance Health Hub leads whenever a brand is referenced. Used by the carousel
  // slide footers AND every platform prompt's CTA, so it must not diverge.
  handle:          process.env.BRAND_SOCIAL_HANDLE   || '@vancehealthhub',
  hashtagExamples: process.env.BRAND_HASHTAG_EXAMPLES|| '#ClinicalNutrition #MedicalFoods #PatientCare',
};
...
// CTA templates support {handle} interpolation, resolved at prompt-build time.
export const CTA_TEMPLATES = {
  grow:    'Follow {handle} for weekly clinical insights',
...
export function buildPlatformPrompt(platform, pillar, ctaGoal, archetype, articleTitle, articleExcerpt) {
  if (!PLATFORM_CONFIGS[platform])   throw new Error(`Unknown platform: "${platform}". Valid: ${Object.keys(PLATFORM_CONFIGS).join(', ')}`);
  if (!PILLAR_INSTRUCTIONS[pillar])  throw new Error(`Unknown pillar: "${pillar}". Valid: ${Object.keys(PILLAR_INSTRUCTIONS).join(', ')}`);
  if (!CTA_TEMPLATES[ctaGoal])       throw new Error(`Unknown ctaGoal: "${ctaGoal}". Valid: ${Object.keys(CTA_TEMPLATES).join(', ')}`);

  const cfg = PLATFORM_CONFIGS[platform];
  const pillarInstruction = PILLAR_INSTRUCTIONS[pillar];
  const ctaText = CTA_TEMPLATES[ctaGoal].replace('{handle}', BRAND.handle);
  ...
  let prompt = `You are a social media expert applying the Ava personal brand methodology for ${BRAND.name}, focused on ${BRAND.niche}.
...
Hashtags: ${cfg.hashtagCount} (relevant to ${BRAND.niche} — e.g. ${BRAND.hashtagExamples})
