...
// Auth Config id per toolkit (ac_… from the Composio dashboard).
const AUTH_CONFIGS = {
  ...
  // Canva is not a posting destination — it is a source of slide artwork for
  // promotional decks (see lib/social/canva.js). It rides the same link() flow
  // because Composio's connection model does not care what a toolkit is for.
  canva:     process.env.COMPOSIO_AUTHCONFIG_CANVA,
};

/** Is an auth config set for this toolkit? Lets a caller distinguish "not set up
 *  yet" from "set up but nothing connected", which need different fixes. */
export function hasAuthConfig(platform) {
  return Boolean(AUTH_CONFIGS[platform]);
}
