...
const PEXELS_API_KEY = process.env.PEXELS_API_KEY || '61644jNcCg5JfspIckM3rGG4lDwaNuUVmbIXCjbxYZDeqGiaNBftxKdJ';
...
// Derive a short Pexels search query from an article title (no LLM). Falls back
// to the cleaned title, then a generic medical query, so we always search for
// something usable.
function heroKeywordsFromTitle(articleTitle) {
  const clean = (articleTitle || '').replace(/^Clinical Review:?\s*/i, '');
  const words = clean.toLowerCase().replace(/[^a-z\s]/g, ' ').split(/\s+/)
    .filter(w => w.length > 4 && !HERO_STOPWORDS.has(w));
  const seen = new Set();
  const kws = [];
  ...
  return kws.length ? kws.join(' ') : (clean.trim() || 'healthcare medicine clinical');
}

/**
 * Fetch a landscape stock photo for the article hero from Pexels — no AI, no
 * LLM round-trip. Returns { url, photographer, sourceUrl, query } or throws.
 * Signature mirrors generateImageFast's return shape ({ url }) so callers can
 * treat the two hero sources interchangeably.
 */
export async function generateHeroStockImage(articleTitle) {
  const query = heroKeywordsFromTitle(articleTitle);
  ...
  if (!url) throw new Error('Pexels photo had no usable src');
  return { url, photographer: photo.photographer || null, sourceUrl: photo.url || null, query };
}
