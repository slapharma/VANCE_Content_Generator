import { filterUnusedStock, markStockUsed } from './stock-ledger.js';
...
// Generic safety nets, tried in order once the title-derived queries are spent.
// There are several rather than one because no photo may be used twice (see
// lib/social/stock-ledger.js): a single generic query is a finite pool of photos
// that eventually runs dry, and each rung here buys another few hundred.
const HERO_GENERIC_QUERIES = [
  'healthcare medicine clinical',
  'medical research laboratory',
  'doctor patient consultation',
  'hospital clinic interior',
  'nutrition healthy food',
  'science microscope research',
];

// Ordered list of stock queries for a title. The article title itself is the
// primary query; each subsequent entry broadens it so automation never ends up
// with no hero image (long/niche titles can return zero results).
function heroStockQueries(articleTitle) {
  ...
  queries.push(...HERO_GENERIC_QUERIES);                             // 4. generic safety nets
  const seen = new Set();
...
// How wide the search goes per query before broadening. No photo may be used
// twice, so a query whose first page is entirely spent has to be able to reach
// deeper rather than immediately giving up on an otherwise good search term.
const HERO_PER_PAGE       = 30;
const HERO_PAGES_PER_QUERY = 2;   // up to 60 candidates per query

// One page of a Pexels search → normalised landscape candidates (never pinged,
// never filtered — the caller decides which one it takes).
async function pexelsSearchPage(query, page) {
  const resp = await fetch(
    `https://api.pexels.com/v1/search?query=${encodeURIComponent(query)}`
      + `&per_page=${HERO_PER_PAGE}&page=${page}&orientation=landscape`,
  ...
  return photos.map((photo) => {
    ...
    return {
      url,
      id: photo.id != null ? String(photo.id) : '',
      provider: 'pexels',
      photographer: photo.photographer || null,
      photographerUrl: photo.photographer_url || null,
      sourceUrl: photo.url || null,
      downloadLocation: null,     // Pexels has no download-ping equivalent
    };
  }).filter(Boolean);
}

// One page of an Unsplash search → normalised landscape candidates. The download
// endpoint is deliberately NOT pinged here: the Unsplash guideline is to ping when
// a photo is actually used, and most of these will be passed over.
async function unsplashSearchPage(query, page, key) {
  ...
  const data = await resp.json();
  const photos = data && Array.isArray(data.results) ? data.results : [];
  return photos.map((photo) => {
...
// Unsplash API guideline: ping the download endpoint for the photo actually taken.
// Fire-and-forget — a failed courtesy ping must never cost us the hero image.
function pingUnsplashDownload(hit, key) {
  const dl = hit && hit.downloadLocation;
  const accessKey = key || UNSPLASH_ACCESS_KEY;
  if (!dl || !accessKey) return;
  fetch(dl + (dl.includes('?') ? '&' : '?') + 'client_id=' + encodeURIComponent(accessKey)).catch(() => {});
}

/**
 * Walk the query ladder for one provider and return the first candidate that has
 * not been used before. `exhausted` distinguishes "this provider had plenty of
 * photos, they are all spent" from "this provider returned nothing / errored",
 * which the caller turns into two different error messages.
 */
async function firstUnusedStockHit(searchPage, queries) {
  let lastErr = null;
  let exhausted = false;
  for (const query of queries) {
    for (let page = 1; page <= HERO_PAGES_PER_QUERY; page++) {
      let candidates;
      try {
        candidates = await searchPage(query, page);
      } catch (e) {
        lastErr = e;
        break;                                     // provider error — next query
      }
      if (!candidates.length) {
        if (page === 1) lastErr = new Error(`no results for "${query}"`);
        break;                                     // out of pages — next query
      }
      const fresh = await filterUnusedStock(candidates);
      if (fresh.length) return { hit: fresh[0], query };
      // Every candidate on this page is already used — go deeper on this query.
      exhausted = true;
    }
  }
  return { hit: null, query: null, lastErr, exhausted };
}

/**
 * Fetch a landscape stock photo for the article hero — no AI, no LLM round-trip.
 * Searches by the article title first, broadening only when a query yields nothing
 * unused. `opts.provider` selects 'pexels' (default) or 'unsplash';
 * `opts.unsplashKey` supplies the Unsplash access key when relevant.
 *
 * No photo is ever used twice: candidates already in the stock ledger are skipped,
 * and the one taken is recorded before this returns (see lib/social/stock-ledger.js).
 * If every candidate across the whole query ladder is spent, this THROWS rather
 * than duplicating — the caller leaves the hero unset and the category fallback
 * image or a manual pick fills it in.
 *
 * If Unsplash exhausts every query (e.g. the 50 req/hr demo-tier cap, or a bad
 * key), falls back to Pexels rather than leaving the article with no hero image —
 * Pexels uses its own built-in key so it's unaffected by the shared Unsplash budget.
 *
 * Returns { url, id, photographer, photographerUrl, sourceUrl, query, provider } or throws.
 */
export async function generateHeroStockImage(articleTitle, opts = {}) {
  ...
  const searchers = provider === 'unsplash'
    ? [['unsplash', (q, p) => unsplashSearchPage(q, p, opts.unsplashKey)], ['pexels', pexelsSearchPage]]
    : [['pexels', pexelsSearchPage]];

  let lastErr = null;
  let exhausted = false;
  for (const [name, searchPage] of searchers) {
    const { hit, query, lastErr: err, exhausted: spent } = await firstUnusedStockHit(searchPage, queries);
    if (hit) {
      if (hit.provider === 'unsplash') pingUnsplashDownload(hit, opts.unsplashKey);
      await markStockUsed(hit);
      return {
        url: hit.url,
        id: hit.id || null,
        ...
        ...(name !== provider ? { fallbackFrom: provider } : {}),
      };
    }
    lastErr = err || lastErr;
  ...
  if (exhausted) {
    throw new Error('every matching stock photo has already been used — hero left unset rather than reusing one');
  }
