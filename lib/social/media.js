...
async function pexelsSearchTop(query) {
  ...
  return url ? { url, photographer: photo.photographer || null, sourceUrl: photo.url || null } : null;
}
...
async function unsplashSearchTop(query, key) {
  ...
  return { url, photographer: (photo.user && photo.user.name) || null, sourceUrl: (photo.links && photo.links.html) || null };
}
