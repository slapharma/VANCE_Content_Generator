 ...
 * LLM round-trip. Searches by the article title first, broadening only if a
 * query returns nothing. Returns { url, photographer, sourceUrl, query } or
 * throws. Signature mirrors generateImageFast's return shape ({ url }) so
 * callers can treat the two hero sources interchangeably.
 */
export async function generateHeroStockImage(articleTitle) {
  ...
  for (const query of queries) {
    try {
      ...
      if (!resp.ok) { lastErr = new Error(`Pexels ${resp.status}`); continue; }
      const data = await resp.json();
      const photo = data && Array.isArray(data.photos) ? data.photos[0] : null;
      if (!photo) { lastErr = new Error(`no Pexels results for "${query}"`); continue; }
      const url = photo.src && (photo.src.large2x || photo.src.large || photo.src.original);
      if (!url) { lastErr = new Error('Pexels photo had no usable src'); continue; }
      return { url, photographer: photo.photographer || null, sourceUrl: photo.url || null, query };
    } catch (e) { lastErr = e; }
  }
  throw lastErr || new Error('Pexels search failed');
}
