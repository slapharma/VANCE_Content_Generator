 ...
 *
 * This constant is the convention's canonical spelling, for naming a new design.
 * Nothing compares against it directly: matching goes through isHouseTitle below,
 * which is deliberately laxer about punctuation than this string is.
 */
export const DESIGN_PREFIX = 'Vance Carousel - ';

/**
 * The search term sent to Canva, and the flattened form matched against.
 *
 * Both derived from DESIGN_PREFIX so the convention has exactly one spelling to
 * change. The search term drops the punctuation because Canva's search is fuzzy:
 * the separator buys no precision there and costs matches, since a title comes
 * back with whichever dash the Canva editor produced rather than the one above.
 */
const HOUSE_QUERY = DESIGN_PREFIX.replace(/[^A-Za-z0-9 ]/g, '').trim();
const HOUSE_KEY = DESIGN_PREFIX.toLowerCase().replace(/[^a-z0-9]/g, '');

/**
 * Does this title follow the house convention?
 *
 * Compared on letters and digits only, which is why this is a function rather
 * than a `startsWith(DESIGN_PREFIX)` at each call site.
 *
 * The separator is not stable across the two APIs that report these titles.
 * Verified 2026-07-31 against one template id: Composio (the path this app
 * actually uses) returns "Vance Carousel - Dashboard" with a hyphen, while the
 * Canva MCP connector returns the same id as "Vance Carousel — Dashboard" with
 * an em dash. Copying a template through the MCP connector then carries the em
 * dash into the copy for real, so the republished 2026-07-31 promo templates
 * genuinely have one and an exact hyphen match would drop every one of them.
 *
 * Flattening the string makes the question moot rather than answering it, which
 * is the point: the app should care what a name says, not how it was punctuated
 * or which middleware last rewrote it. Same normalisation `buildAutofillData`
 * uses on field names below.
 *
 * Flattening the string means the separator stops mattering at all — hyphen, en
 * dash, em dash and stray spacing all disappear. It is the same normalisation
 * `buildAutofillData` uses on field names below, for the same reason: the app
 * should care what a name says, not how it was punctuated.
 *
 * Something must follow the prefix, so a design titled just "Vance Carousel" is
 * still rejected, exactly as the old separator test rejected it.
 */
export function isHouseTitle(title) {
  const flat = String(title || '').toLowerCase().replace(/[^a-z0-9]/g, '');
  return flat.startsWith(HOUSE_KEY) && flat.length > HOUSE_KEY.length;
}
...
export async function listBrandTemplates({ query = '', continuation = null } = {}) {
  ...
  const args = {
    ownership: 'any',
    dataset: 'non_empty',
    query: searching ? query : HOUSE_QUERY,
  };
  if (continuation) args.continuation = continuation;
  const out = body(await call(SLUGS.listTemplates, args));
  return {
    items: (out?.items || [])
      .filter((t) => searching || isHouseTitle(t.title))
      .map((t) => ({
      id: t.id,
...
export function buildAutofillData(spec, fields) {
  ...
  // `subhead` and friends sit with the brief because that is what they are on a
  // promo deck: the sentence under the headline explaining the offer. The four
  // hand-built promo decks declare `subhead` on their cover, and before this
  // alias existed it matched nothing.
  //
  // What that actually looked like is worth recording, because it is quieter
  // than it sounds. An unfilled autofill field keeps whatever text the design
  // already held, so the cover kept its hand-written line while the headline
  // above it was replaced by generated copy (verified 2026-07-31 against
  // EAHQ8lfv6lg through the live preview endpoint). Nothing looks broken; the
  // slide is simply stale and off-message, which is harder to notice than an
  // obviously wrong slide would be.
  put(['brief', 'body', 'intro', 'subhead', 'subheading', 'subtitle'], spec?.brief?.body);
  put(['update', 'close', 'closing'], spec?.update?.body);
  put(['cta', 'ctaLabel'], spec?.cta?.label);
  put(['domain'], spec?.cta?.domain);
  put(['note', 'ctaNote'], spec?.cta?.note);
  ...
  // Columns from an operator's CSV whose names match nothing this app models.
  // They are still real fields in somebody's template, so they are sent under
  // their own spelling. Last, so a column always beats a generated value it
  // collides with: the operator wrote it down, which settles the question.
  for (const [name, text] of Object.entries(spec?.customFields || {})) put([name], text);
